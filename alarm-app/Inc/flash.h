...
#define DATA_BYTE            ((uint8_t)0x44)  /* 8-bits value to be programmed */
...
void LockNVM(void);
//void EEPROM_WriteBytes(uint32_t Addr,uint32_t *Buffer,uint32_t Length);
...
void ConfigureTIM21OutputCompare(void);
void ConfigureTIM2PWM_Input(void);
void ConfigureTIM2PWM_DMA(void);
void  Para_Read(void);
void  Para_Write(uint8_t type,uint16_t *Dat);
